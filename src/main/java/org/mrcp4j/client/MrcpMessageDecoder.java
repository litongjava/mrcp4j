...
public class MrcpMessageDecoder {
  ...
  public MrcpMessage decode(BufferedReader in) throws IOException, ParseException {
    ...
    while ((line = in.readLine()) == null || (line = line.trim()).equals("")) {
      log.debug((line == null) ? "MrcpMessageDecoder: null line" : "MrcpMessageDecoder: empty line");
    }
    ...
    if (startLineParts.length != START_LINE_PART_COUNT) {
      throw new ParseException("Incorrect start-line format, got " + startLineParts.length + " parts in '" + line + "'",
          -1);
    }
    ...
    if (contentLength > 0) {
      ...
      if (remainingBytes < 0) {
        log.warn("read " + -remainingBytes + " more bytes than we were supposed to!");
      }
